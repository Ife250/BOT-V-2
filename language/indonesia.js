...
exports.rules = (prefix) => {
    return `
*── RULES AND FAQ ──* 1. Don't Spam Here: *❎ WARN/SOFT BLOCK* 2. Don't call bots. ️ Sanctions: *❎ SOFT BLOCK* 3. Don't exploit bots.😖 Sanctions: *PERMANENT BLOCK* kingsley Bot not or slow to respond ? ️ May be affected by network, signal, banned by Whatsapp and some random. Keep obeying the rules‼️ ️ Where can I get the script from this bot? ️ This script is still private and has never been traded, be wise in knowing fraudsters. ️ Can I add to the group? ️ The bot is temporarily in free to add status when u ask kingsley to approve. ️ What's the prefix? ️ This bot uses multi prefix. That means you can use the prefix #, . , and other reasonable prefixes. ️ Sis, why is the chat owner not responding? ️ ღᏦíñᎶsᏞéᎽღ only responds to questions about bots and error problems, not for acquaintances or begging for scripts and adding of groups. If you understand the rules, please type *${prefix}allmenu* to get started! ️ All kingsley bot policies and conditions are held by the owner and all policy changes, at any time the owner has the right to revoke, block users (*﹏*) Thank you very much! For you, friendly users and some people who also helped in the kingsley BOT V.2 creation project 🙏 ` } exports.welcome = () =>{ return`Welcome @user  𝗋𝗎𝗅𝖾𝗌 k ~` } exports.leave = () =>{ return `kingsley bot says goodbye and hope ro meet again!` } exports.source = () =>{ return`*------「 SOURCE CODE ------* Base : https://github.com/ife250/BOT-V-2 Recode : https://youtube.com/c/lltrcker ` } exports.tos = (ownernumber) => { return ` *-------「 DONATE -------* Hi Users ️ You can support me to keep this bot up to date by donating Any amount of your donation will mean a lot 👍 Thanks! Contact person Owner: wa.me/${owner number} `
}
